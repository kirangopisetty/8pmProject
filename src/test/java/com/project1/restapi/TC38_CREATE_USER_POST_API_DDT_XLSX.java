...
public class TC38_CREATE_USER_POST_API_DDT_XLSX {
	...
	public Object[][] DDT() throws IOException {
		...
		int rowNum = TC37_XLUTILS.getRowsCount(xlfile, xlsheet);
		int colCount = TC37_XLUTILS.getColumnsCount(xlfile, xlsheet);
		
		String empdata[][]=new String[rowNum][colCount];
		for (int i=1;i<=rowNum;i++) {
			for (int j=0;j<colCount;j++) {
				empdata[i-1][j] = TC37_XLUTILS.getCellData(xlfile, xlsheet,i,j);
			}
		}
		return empdata;
	}
